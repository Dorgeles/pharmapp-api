...
import com.wdyapplications.pharmapp.utils.enums.ExtensionEnum;
...
public class EsUtilities {
	...
	private static Logger slf4jLogger = LoggerFactory.getLogger(EsUtilities.class);
	...
	private static String getStringImage(File file) throws IOException {
	...
	public static String convertToBase64(String pathFichier) throws IOException {
	...
	public static String convertFileToBase64(String pathFichier) throws IOException {
	...
	public static String[] getIndexesArray(String start, String end, SimpleDateFormat dateFormat, String root,
			SimpleDateFormat indexDateFormatter) throws Exception {
		if (dateFormat == null) {
		...
		List<String> indexes = EsUtilities.getIndexBetweenDates(d1, d2, root, indexDateFormatter);
	...
	public static boolean isDateToDB(String date) {
		try {
			String simpleDateFormat = "dd/MM/yyyy";
			DateFormat df = new SimpleDateFormat(simpleDateFormat);
			df.setLenient(false);
	...
	public static boolean isDateToDBorangeMoney(String date) {
		try {
	...
	public static String saveFiles(String fileName, String extension, XSSFWorkbook workbook, ParamsUtils paramsUtils)
		    ...
		    if (!fileIsImage(fileName) && !fileIsTexteDocument(fileName) && !fileIsVideo(fileName)) {
		      System.out.println("  L'extension '" + extension + "' n'est pas prise en compte !  ");
		      return null;
		    }
...
//	}
	...
	public static String getFileUrlLink(String fileName, ParamsUtils paramsUtils) {
	    ...
	    if (file.length > 0 && j < file.length) {
	      ...
	      if (fileIsImage("file." + fileExtension)) {
	        suitableFileDirectory = paramsUtils.getImageLink();
	      } else {
	        if (fileIsTexteDocument("file." + fileExtension)) {
	          suitableFileDirectory = paramsUtils.getTextLink();
	        } else {
	          if (fileIsVideo("file." + fileExtension)) {
	            // suitableFileDirectory = paramsUtils.getVideoLink();
	          }
	        }
	      }
	    }
